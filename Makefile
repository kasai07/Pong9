...
export TARGET	:=	Pong9
BUILD		:=	build
SOURCES		:=	source source/fatfs source/abstraction source/images source/arm11bg
DATA		:=	data
INCLUDES	:=	source source/fatfs source/images source/arm11bg
...
release:
	...
	@[ -d $(RELEASE)/Launcher9 ] || mkdir -p $(RELEASE)/Pong9
	...
	@-[ "$(TARGET)" != "Pong9" ] || cp $(OUTPUT).bin $(STARTER)/extstarterpack/arm9payloads
	@-[ "$(TARGET)" = "Pong9" ] || (([ -d $(STARTER)/extstarterpack/3DS/$(TARGET) ] || mkdir $(STARTER)/extstarterpack/3DS/$(TARGET)) && cp $(RELEASE)/3DS/$(TARGET)/*.* $(STARTER)/extstarterpack/3DS/$(TARGET))
