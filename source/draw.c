...
//SET_PIXEL(TOP_SCREEN0, x1, y, COLOR_WHITE);
void drawLine(int x1, int y1, int x2, int y2, int color)
{
	...
	if (x1 == x2)
	{
		if (y1 < y2) {for (y = y1; y <= y2; y++) SET_PIXEL(TOP_SCREEN0, x1, y, color);}
		else{for (y = y2; y <= y1; y++) SET_PIXEL(TOP_SCREEN0, x1, y, color);}
	}
	else {
		if (x1 < x2) {for (x = x1; x <= x2; x++) SET_PIXEL(TOP_SCREEN0, x, y1, color);}
		else {for (x = x2; x <= x1; x++) SET_PIXEL(TOP_SCREEN0, x, y1, color);}
	}
}
...
void drawFillRect(int x1, int y1, int x2, int y2, int color)
{
	...
	for (i = X1; i <= X2; i++){
		for (j = Y1; j <= Y2; j++){
			SET_PIXEL(TOP_SCREEN0, i, j, color);
		}
	}
}